...
function validateResolverObject(resolverKey, resolverObject) {
  ...
  if (resolverObject.__resolveReference == null) {
    throw new Error(
      `buildFederatedService: options resolvers '${resolverKey}' value must have a reference resolver`
    )
  }
}
...
function validateUrl(url) {
  ...
  if (typeof url !== 'string') {
    throw new Error('buildFederatedService: url must be a string')
  }
}
