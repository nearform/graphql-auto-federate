...
/**
 * wrap every resolver adding the client for making queries to the target service
 * tailored for __resolveReference but can be used in other resolvers too
 */
function injectForwardClient(resolvers, client) {
  for (const typeName of Object.keys(resolvers)) {
    const f = resolvers[typeName]
    if (typeof f === 'function') {
      resolvers[typeName] = async (...args) => f(...args, client)
    } else if (typeof f === 'object') {
      injectForwardClient(resolvers[typeName], client)
    }
  }
}
...
function buildFederatedResolvers({ url, schema, resolvers }) {
  ...
  injectForwardClient(federatedResolvers, bindForwardClient(url))
  return federatedResolvers
}

function validateOptions({ url, options }) {
  if (!url) {
  ...
  if (typeof options.auto !== 'boolean') {
    throw new Error('buildFederatedService: auto must be a boolean')
  }
}

async function buildFederatedService({ url, options: userOptions }) {
  const defaultOptions = { auto: true }
